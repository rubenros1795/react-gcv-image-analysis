...
class LocalPages():
    ...
    def Importer(results_file_path):
        base_path = os.getcwd()
        print("Looking for results.txt in {}".format(os.path.join(base_path, results_file_path, "html", "results.txt")))
        with open(os.path.join(base_path, results_file_path, "html", "results.txt")) as f:
            ...
            for u in urls:
                try:
                    ...
                    id = id + '.html'
                    id = os.path.join(base_path, results_file_path, 'html',id)
    ...
    def Parser(results_file_path, preprocess=True):
        ...
        print("Extracting Full Texts of {} HTML files".format(len(urls)))
        d = dict()

        for c,u in enumerate(urls):
            if c%1000==0:
                print("Now at number {}".format(c))
            ...
            d.update({fn + "_" + url:text})

        d = pd.DataFrame(list(d.items()), columns = ['id', 'text'])
        destination_path = os.path.join(base_path, results_file_path, "txt")
